...
def setup_logger(logfile_path:str = None, log_level=logging.INFO):
    ...
    if logfile_path is None:
        ...
        logfilename = Path(logfile_path).name
    if not Path(logfile_path).parent.exists():
        os.makedirs(Path(logfile_path).parent.resolve(), exist_ok=True)
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.StreamHandler(stream = sys.stderr), # sys.stdout),
            logging.FileHandler(logfile_path),  #(logfile_path),
        ],
    )
    return logfilename, logfile_path
...
def generate_inbreast_metapoints(
    ...
    for indx, c in enumerate(contours):
        if c.shape[0] < 2:
            ...
            if filter_duplicates and is_metapoint_duplicate(lesion_metapoint=metapoint, metadata=lesion_metapoints):
    ...
    for indx, c in enumerate(contours):
        if c.shape[0] < 2:
            ...
            metapoint = Metapoint(
                ...
                image_id=image_id,
                patient_id=patient_id,
                predefined_set="training" if "training" in str(image_path.relative_to(CBIS_DDSM_ROOT_PATH)).lower() else "test",
                ...
                dataset="cbis-ddsm",
                contour=np.squeeze(c).tolist(),
                subtlety=int(csv_metadata["subtlety"]),
                mass_margins='unknown' if 'mass' not in roi_type.lower() else str(csv_metadata["mass margins"]),
                mass_shape='unknown' if 'mass' not in roi_type.lower() else str(csv_metadata["mass shape"]),
            )
            if filter_duplicates and is_metapoint_duplicate(lesion_metapoint=metapoint, metadata=lesion_metapoints):
                continue  # not adding this metapoint to avoid duplicates.
            patch_id += 1
...
def is_metapoint_duplicate(lesion_metapoint:dict, metadata:list):
    image_id = Path(lesion_metapoint.image_path).name
    image_bbox = lesion_metapoint.bbox
    # Checking if primary key consisting of image_id and bbox is already present in metadata database.
    return any([True for metapoint in metadata if (Path(metapoint.image_path).name == image_id and metapoint.bbox == image_bbox)])
...
def get_mask_from_metapoint(metapoint: Metapoint) -> np.ndarray:
    ...
    if metapoint.contour is not None:
        mask = cv2.fillPoly(
            mask,
            pts=[np.array(metapoint.contour, dtype=np.int32).reshape((-1, 2))],
            color=255,
        )

    elif not metapoint.is_healthy:  # don't draw a mask for healthy bboxes

        logging.debug("Mask missing, using bbox as a mask")
...
def generate_radiomics(
    ...
    # https://pyradiomics.readthedocs.io/en/latest/customization.html#feature-extractor-level
    ...
    logging.info(f"Computing radiomics features for metadata: {features_to_compute}")
