...
class GANConfig(BaseConfig):
    ...
    # learning rate of a second discriminator
    lr_d2: float = 0.0001

    # Is D2 pretrained during GAN training? If so, the pretrain_objectives list will be used to determine the nature of the pretraining
    ...
    # Set to True to use nn.InstanceNorm2d layers in D. Else: No normalization (or BatchNorm2D) is used
    ...
    def __post_init__(self):
        super().__post_init__()
        if self.conditional and self.nc % 2 != 0:
            # check if conditional model training. If the number of channels is already %2==0, then condition was already
            # accounted for. This is relevant when running the generate.py methods after training
            self.nc = self.nc + 1
