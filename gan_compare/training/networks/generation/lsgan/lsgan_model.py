...
class LSGANModel(DCGANModel):
    def __init__(self, config: GANConfig, dataloader: DataLoader, use_dp: bool=False, is_visualized:bool=True):
        super(LSGANModel, self).__init__(config=config, dataloader=dataloader, use_dp=use_dp)
        self.config = config
    ...
    def periodic_training_console_log(
        ...
        **kwargs,
        ...
        if self.config.pretrain_classifier:
            ...
            logging.info(
                "[%d/%d][%d/%d]\tLoss_D1: %.4f\tLoss_D2: %.4f\tLoss_G_D1: %.4f\tLoss_G_D2: %.4f\tD(x): %.4f\tD(G(z1)): %.4f\tD(G(z2)): %.4f\tD2(x): %.4f\tD2(G(z1)): %.4f\tD2(G(z2)): %.4f \t %s "
                % (
                    epoch,
            ...
            logging.info(
                "[%d/%d][%d/%d]\tLoss_D: %.4f\tLoss_G: %.4f\tD(x): %.4f\tD(G(z1)): %.4f\tD(G(z2)): %.4f \t %s "
                % (
                    epoch,
                    self.config.num_epochs - 1,
                    iteration,
