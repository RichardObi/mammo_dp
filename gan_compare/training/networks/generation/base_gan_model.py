...
class BaseGANModel:
    def __init__(
        ...
        # Differential Privacy (DP-SGD) injected during training
        self.use_dp = use_dp
        ...
        self.device = torch.device(
            "cuda" if (torch.cuda.is_available() and self.config.ngpu > 0 and device != 'cpu') else "cpu"
        )
    ...
    def _assert_network_channels(self):
        ...
        if self.config.conditional:
            ...
            ), f"To use conditional input, change number of channels (nc={self.config.nc}) to 2 (default) or 4 (swin transformer)."
        else:
            assert self.config.nc == 1 or (
                self.config.nc == 3 and self.config.model_name == "swin_transformer"
            ), f"Without conditional input into GAN, change number of channels (nc={self.config.nc}) to 1 (default) or 3 (swin transformer)."
    ...
    def network_setup(
        ...
        if self.use_dp or self.config.use_dp:
            netD = only_use_opacus_supported_layers(netD)
        logging.info(netD)

        netG = self._network_weights_init(net=netG)
        netG = self._handle_multigpu(net=netG)
        #if self.use_dp or self.config.use_dp:
        #    netG = only_use_opacus_supported_layers(netG)
    ...
    def handle_G_updates(
        ...
        if (
            ...
            if self.config.pretrain_classifier:
                self.netG.zero_grad()
                self.optimizerG.zero_grad()  # More info: https://discuss.pytorch.org/t/zero-grad-optimizer-or-net/1887/10
    ...
    def train(self):
        ...
        if self.use_dp or self.config.use_dp:  # for ease of use, we want to allow activating DP both via args as well as via config. Both have default=False
            netD, optimizerD, dataloader, privacy_engine = make_private_with_epsilon(net=self.netD, optimizer=self.optimizerD,
        ...
        # Training Loop
        logging.info(
            f"Starting Training on {self.device}. Image size: {self.config.image_size}. On Data: {self.config.data}. {str('Target DP(ε='+str(self.config.dp_target_epsilon)+',δ='+str(self.config.dp_target_delta)+') with max_grad_norm='+str(self.config.dp_max_grad_norm)) if self.config.use_dp or self.use_dp else ''}"
        ) if not self.config.conditional else logging.info(
            f"Starting Training on {self.device}. Image size: {self.config.image_size}. On Data: {self.config.data}. Training conditioned on: "
            f"{self.config.conditioned_on if self.config.conditioned_on is not None and self.config.conditioned_on in ['density', 'birads'] else f'{self.config.training_target}={self.config.classes}'}"
            f" {('as a continuous (+ random noise: ' + f'{self.config.added_noise_term})' + ' and not') * (1 - self.config.is_condition_categorical)}as a categorical variable. "
            f"{str('Target DP(ε='+str(self.config.dp_target_epsilon)+',δ='+str(self.config.dp_target_delta)+') with max_grad_norm='+str(self.config.dp_max_grad_norm)) if self.config.use_dp or self.use_dp else ''}"
        )
        ...
        for epoch in range(self.config.num_epochs):
            ...
            for i, data in enumerate(self.dataloader, 0):
                ...
                # We also want to clear the optimizer in case our optimizer has less/different model parameters
                # This is only the case in our Differential Privacy setting.
                # Further info: https://discuss.pytorch.org/t/zero-grad-optimizer-or-net/1887/10
                ...
                if self.config.pretrain_classifier:
                    ...
                    self.optimizerD2.zero_grad() # More info: https://discuss.pytorch.org/t/zero-grad-optimizer-or-net/1887/10
                    are_outputs_logits = (
                        True if self.config.model_name == "swin_transformer" else False
                    )
                    (
                ...
                if i % self.config.num_iterations_between_prints == 0:
                    if self.config.use_dp or self.use_dp:
                        epsilon = privacy_engine.get_epsilon(self.config.dp_target_delta)
                    self.periodic_training_console_log(
                        ...
                        DP=f"Current DP(ε={epsilon},δ={self.config.dp_target_delta})"
                    )
