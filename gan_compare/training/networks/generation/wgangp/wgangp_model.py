...
class WGANGPModel(BaseGANModel):
    def __init__(self, config: GANConfig, dataloader: DataLoader, use_dp: bool=False, is_visualized:bool = True):
        super(WGANGPModel, self).__init__(config=config, dataloader=dataloader, use_dp=use_dp)
        self.config = config
    ...
    def periodic_training_console_log(
        ...
        **kwargs,
        ...
        if self.config.pretrain_classifier:
            ...
            logging.info(
                "[%d/%d][%d/%d] \t Loss_D: %.4f \t Loss_G: %.4f \t D(x): %.4f \t D(G(z1)): %.4f \t D(G(z2)) %.4f \t Gradient Penalty: %.4f \t D2(x): %.4f\tD2(G(z1)): %.4f\tD2(G(z2)): %.4f \tAcc(D2(x)): %.4f\tAcc(D2(G(z)): %.4f \t %s "
                % (
                    epoch,
            ...
            logging.info(
                "[%d/%d][%d/%d] \t Loss_D: %.4f \t Loss_G: %.4f \t D(x): %.4f \t D(G(z1)): %.4f \t D(G(z2)) %.4f \t Gradient Penalty: %.4f \t %s "
                % (
                    epoch,
                    self.config.num_epochs - 1,
                    iteration,
