...
class BaseDataset(Dataset):
    ...
    def __init__(
        ...
        transform: any = None,
        # sampling_ratio: float = 1.0, #Outcommented, as mammo_dataset.py has separate filtering that needs to be applied before sampling_ratio filtering.
        normalize_output: bool = True,  # applies only to regression scenario
        subset: str = "train",
    ):
        assert (
        ...
        if config.is_regression and self.normalize_output:
            self.normalize_output_data(self.metadata_unfiltered)
    ...
    def len_of_classes(self):
        ...
        for d in self.metadata:
            if type(d) is str and "is_healthy" in self.config.classes:
                continue  # then d is a synthetic sample (therefore non-healthy)
            elif getattr(d, self.config.classes):
                cnt += 1
        return len(self) - cnt, cnt

    def arrange_weights_healthy(self, weight_non_healthy, weight_healthy):
        return [
            weight_healthy
    ...
    def remove(self, remaining_indices: list):
        # Warning, calling this method changes the index of self.metadata.
        metadata = [self.metadata[index] for index in remaining_indices]
        assert len(metadata) < len(self.metadata), f"Failed to reduce the len of metadata. Previously had {len(self.metadata)} samples, now there are  {len(metadata)}."
    ...
    def determine_label(self, metapoint: Metapoint) -> Union[int, float, np.ndarray]:
        ...
        logging.debug(f"target: {target}, type(target): {type(target)}")
        if type(target) == int or type(target) == float:
            return target
        if type(target) == dict:
            if (
                self.config.training_target == "radiomics"
                and self.config.target_radiomics_features is not None
            ):
                # get only the targeted selected radiomics features from the metapoint instead of all.
                return np.asarray(
                    [target[x] for x in self.config.target_radiomics_features]
                )
            return np.asarray(list(target.values()))
        if type(target) == bool:
            return int(target)
        # if (
        #         hasattr(self.config, "binary_classification") and self.config.binary_classification
        ...
        if self.config.classes == "is_healthy":
            return int(metapoint.is_healthy)  # label = 1 if metapoint is healthy
        if self.config.classes == "is_benign":
            return int(
                metapoint.is_benign
            )  # label = 1 (True) if metapoint is benign, 0 (False) if malignant, else -1
    ...
    def normalize_output_data(self, metadata):
        data = [
        ...
        if self.config.training_target == "radiomics":
            self.norm_feature_params = RADIOMICS_NORMALIZATION_PARAMS

        # compute (additional missing) normalization parameters (mean, variance)
        for feature in data[0].keys():
            if feature not in self.norm_feature_params.keys():
                ...
                print(feature + ": " + str(self.norm_feature_params[feature]))
        ...
        for metapoint, norm_sample in zip(metadata, data):
            setattr(metapoint, self.config.training_target, norm_sample)
